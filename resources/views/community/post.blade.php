...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .comment-composer.is-focused .comment-submit,
        .comment-composer:focus-within .comment-submit { display: inline-flex; }
            ...
            <div class="best-box">
                @foreach($post->comments->whereIn('id', $bestCommentIds) as $bestComment)
                    <article class="comment" style="padding-top: 24px; cursor: pointer;" onclick="navigateToCommentDetail(event, {{ $post->id }}, {{ $bestComment->id }}, '{{ $apartmentId }}');">
                        ...
                        @php($bestCommentMsgId = (($isVerifiedUser ?? false) && ! $bestComment->is_anonymous && $bestComment->user && $bestComment->user->id !== $currentUserId) ? $bestComment->user->id : null)
                        ...
                        <div class="comment-body">
                            ...
                            @if($bestComment->image_path)<img class="comment-image" src="{{ $bestComment->image_path }}" alt="첨부한 댓글 사진">@endif
                            ...
                            <div class="comment-actions" onclick="event.stopPropagation();">
                                ...
                                @endif
                                @include('community.partials.comment-report-menu', ['commentId' => $bestComment->id, 'commentUserId' => $bestComment->user_id, 'apartmentId' => $apartmentId, 'currentUserId' => $currentUserId, 'redirectUrl' => '/community/posts/'.$post->id.'?apartment_id='.$apartmentId])
                            </div>
                                ...
                                <div class="children" style="margin-top:24px;">
                                    @foreach($bestVisibleChildren as $child)
                                        <article class="comment" style="grid-template-columns: 22px 1fr; padding-top:24px;">
                                            ...
                                            @php($bestChildMsgId = (($isVerifiedUser ?? false) && ! $child->is_anonymous && $child->user && $child->user->id !== $currentUserId) ? $child->user->id : null)
...
@if($canComment)
    @include('community.partials.comment-composer')
@endif
<!--
