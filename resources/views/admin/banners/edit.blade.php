...
<div style="max-width: 800px; margin: 0 auto; padding: 24px 16px;">
    ...
    <form method="post" action="{{ route('admin.banners.update', $banner) }}" style="background: #fff; border: 1px solid #e0e6ed; border-radius: 8px; padding: 24px;" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="title" value="{{ old('title', $banner->title) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
        </div>

        <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #15243a;">배너 설명</label>
            <textarea name="description" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem; min-height: 80px; resize: vertical;">{{ old('description', $banner->description) }}</textarea>
        </div>
        ...
        <div style="margin-bottom: 20px; display: {{ old('type', $banner->type) === 'image' ? 'block' : 'none' }};" id="image-field">
            ...
            <input type="url" name="image_url" value="{{ old('image_url', $banner->image_url) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            <p style="margin: 8px 0 0; color: #607086; font-size: 0.9rem;">또는</p>
            <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #15243a; margin-top: 12px;">이미지 파일 업로드 (새 파일 선택 시 기존 파일 대체)</label>
            ...
            @if($banner->image_path)
                <div style="margin-top: 8px; padding: 8px; background: #f0f2f5; border-radius: 4px; font-size: 0.85rem; color: #607086;">
                    현재 파일: {{ basename($banner->image_path) }}
                </div>
            @endif
        </div>
        ...
        <div style="margin-bottom: 20px; display: {{ old('type', $banner->type) === 'video' ? 'block' : 'none' }};" id="video-field">
            ...
            <input type="url" name="video_url" value="{{ old('video_url', $banner->video_url) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            <p style="margin: 8px 0 0; color: #607086; font-size: 0.9rem;">또는</p>
            <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #15243a; margin-top: 12px;">영상 파일 업로드 (새 파일 선택 시 기존 파일 대체)</label>
            ...
            @if($banner->video_path)
                <div style="margin-top: 8px; padding: 8px; background: #f0f2f5; border-radius: 4px; font-size: 0.85rem; color: #607086;">
                    현재 파일: {{ basename($banner->video_path) }}
                </div>
            @endif
        </div>
        ...
        <div style="margin-bottom: 20px; display: {{ old('type', $banner->type) === 'text' ? 'block' : 'none' }};" id="text-field">
            ...
            <textarea name="text_content" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem; min-height: 100px; resize: vertical;">{{ old('text_content', $banner->text_content) }}</textarea>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
            <div>
                <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #15243a;">배너 링크 URL</label>
                <input type="url" name="button_url" value="{{ old('button_url', $banner->button_url) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $banner->sort_order) }}" min="0" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date', $banner->start_date?->format('Y-m-d')) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            </div>
            <div>
                <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #15243a;">노출 종료일</label>
                <input type="date" name="end_date" value="{{ old('end_date', $banner->end_date?->format('Y-m-d')) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #d6e0ea; border-radius: 6px; font-family: inherit; font-size: 1rem;">
            </div>
        </div>
        ...
        <div style="display: flex; gap: 12px;">
            <button type="submit" class="btn btn-primary">배너 수정</button>
